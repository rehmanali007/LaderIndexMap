...
def process_lader_index_map(input_file, output_file, code):
    ...
    cmd = f"gdal_calc.py -A gdal_calc.py --NoDataValue=0 --outfile=F_temp.tif -A {input_file} --calc={calc}"
    logging.info(f"Running: {cmd}")
    os.system(cmd)

    cmd = "gdalwarp -tr 300 300 -dstnodata 0 -te -13887126.77 2814162.483 -7430226.771 6360162.483 -t_srs EPSG:3857 -overwrite -r bilinear F_temp.tif  G_temp.tif"
    logging.info(f"Running: {cmd}")
    os.system(cmd)
    ...
    cmd = 'saga_cmd grid_tools "Reclassify Grid Values" -INPUT=G_temp.tif -RESULT=H_temp.sdat -RETAB=matrix.txt -METHOD=0 -OLD=0.0 -NEW=0.0 -SOPERATOR=0 -MIN=0.0 -MAX=1.0 -RNEW=2.0 -ROPERATOR=0 -RETAB -OTHEROPT=false -OTHERS=0.0'
    logging.info(f"Running: {cmd}")
    os.system(cmd)

    # The command below probably needs testing manually... -TOPERATOR=0 probably.
    cmd = 'saga_cmd io_gdal "Export GeoTIFF" -GRIDS=H_temp.sdat -TOPERATOR=0 -NODATAOPT=true -FILE=J_Temp.tif'
    logging.info(f"Running: {cmd}")
    os.system(cmd)
    ...
    cmd = f"gdal_translate -tr 300 300 -a_nodata 0 -co  COMPRESS=LZW -co JPEG_QUALITY=80 J_Temp.tif {output_file}"
    logging.info(f"Running: {cmd}")
    os.system(cmd)
